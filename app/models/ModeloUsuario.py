...
#             data = cursor.fetchone()
#             # print(data)
#             coincide = Usuario.encriptar_password(data[2], usuario.password)
#             if coincide:
#                 usuario_logeado = Usuario(data[0], data[1], None, None)
#                 return usuario_logeado
#             else:
...
#         try:
#             cursor=db.connection.cursor()
#             sql=f"SELECT USU.id, USU.usuario, TIP.id, TIP.nombre FROM usuario USU JOIN tipousuario TIP ON USU.tipousuario_id = TIP.id WHERE USU.id ={id}"
#             cursor.execute(sql)
#             data = cursor.fetchone()
#             tipousuario = TipoUsuario(data[2], data[3])
#             usuario_logeado = Usuario(data[0], data[1], None, tipousuario)
